...
// Helper to convert node-style functions to an async-style generator.
// Example:
//   FS.readdir(x, function(err, result) {
//     doStuff(result);
//   });
// Becomes:
//   var result = yield Async.asyncifyNode(FS.readdir)(x);
//   doStuff(result);
function asyncifyNode(nodeFunc) {
  return function*(n_args) {
    const caller = yield;
    var args = Array.prototype.splice.call(arguments, 0);
    args.push(function(err, result) {
      if (err) caller.failure(err);
      else caller.success(result);
    });
    nodeFunc.apply(undefined, args);
  };
}

function asyncifyNodeModule(module) {
  if (module.asyncified)
    return module;
  module.asyncified = true;
  for (var name in module)
    module[name + "Async"] = asyncifyNode(module[name]);
  return module;
}

function* sleep(ms) {
  const caller = yield;
  setTimeout(caller.success, ms);
}

exports.call = call;
exports.cb = cb;
exports.asyncifyNode = asyncifyNode;
exports.asyncifyNodeModule = asyncifyNodeModule;
exports.sleep = sleep;
exports.test = function() {
  ...
  call(function*() {
    ...
    try {
      var version = yield asyncifyNode(FS.readFile)("JUNKFILE");
    } catch(e) {
      console.log("caught asyncifyNode", e);
    }

    var FS = require("fs");
    var version = yield asyncifyNode(FS.readFile)("LATEST");
    console.log("read file:", version.toString());
