...
function runGenObj(genObj, callbacks) {
  ...
  function runYieldedValue(yieldedValue, subCallbacks) {
    if (yieldedValue === undefined) {
      ...
      runGenObj(yieldedValue, subCallbacks);
...
exports.test = function() {
  ...
  run(function*() {
    ...
    console.log(yield toPromise(echo("in promise")));
    try {
    ...
    try {
      console.log(yield toPromise(echo("failed promise3", 200)));
    } catch(e) {
