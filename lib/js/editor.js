...
function init() {
  ...
  gTemplate.itemSetDelete = function(event, detail, sender) {
    var champion = gTemplate.champion;
    var current = gTemplate.$.itemSetTabs.selected;
    var removed = champion.itemSets.splice(current, 1);
    if (current >= champion.itemSets.length)
      gTemplate.$.itemSetTabs.selected = current-1;

    champion._lisa_deletedItemSets = champion._lisa_deletedItemSets || [];
    champion._lisa_deletedItemSets.push(removed[0]);
  };
