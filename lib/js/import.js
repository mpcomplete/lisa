...
function init() {
  ...
  gTemplate.importFileTapped = function() {
    ...
    Async.runGenObj(importImpl(ImportLocal.importZipFile(elem.value)));
  };
  gTemplate.importClientTapped = function() {
    Async.runGenObj(importImpl(ImportLocal.importClient()));
  };
...
function importFromUrl(url) {
  ...
  Async.runGenObj(importImpl(importer, true));
}
  ...
  try {
    ...
    if (isSingleParser) {
      ...
      for (var name in champions) {
        generators.push(window.loadChampion(name).then(function(champion) {
