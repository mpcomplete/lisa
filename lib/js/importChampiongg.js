...
var kBaseUrl = "http://champion.gg/champion/";
var kErrorMessage = "Page at URL doesn't look like a champion.gg build guide.";

function* importAllChamps() {
  var champions = ChampionTable.getChampionList();

  for (var i = 0; i < champions.length; i += 5) {
    var batch = champions.slice(i, i+5);
    var generators = batch.map(function(champion) {
      return kBaseUrl + champion.dir; // importUrl
    });
    console.log("import:", generators);
    yield Async.sleep(2000);
  }
  return true;
}
...
exports.importAllChamps = importAllChamps;
