...
function initMain() {
  ...
  gTemplate.revertTapped = Async.cb(function*() {
    ...
    yield generators;
    yield selectChampion(gTemplate.champion.name, false);
  });
  ...
  $(".championPicker.selectized").on("change", Async.cb(function*() {
    yield selectChampion($(this).val(), true);
  }));
  gTemplate.selectChampion = Async.cb(function*(event, detail, sender) {
    yield selectChampion(sender.selectedModel.openChampion.name, true);
  });
}
...
function* finalizeMain() {
  ...
  yield selectChampion(Settings.getCurrentChampion(), false);
}
...
function* loadChampion(championName) {
  if (gOpenChampionsMap[championName])
    return gOpenChampionsMap[championName];

  var champion = {name: championName};
  gOpenChampionsMap[championName] = champion;
  gTemplate.openChampions.push(champion);
