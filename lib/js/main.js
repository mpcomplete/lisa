...
function* selectChampionGen(championName, silent) {
  ...
  if (!gOpenChampionsMap[championName])
    yield loadChampionGen(championName);
...
function loadChampion(championName) {
  return Async.toPromise(loadChampionGen(championName));
}

function* loadChampionGen(championName) {
  if (!gOpenChampionsMap[championName]) {
  ...
  deepObserve(champion, function(changes) {
    if (changes.length == 1 && changes[0].name == "_lisa_changed")
      return;
    console.log("champ changed:", changes);
