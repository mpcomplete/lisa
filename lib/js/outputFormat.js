...
function addSkillBlocks(itemSet, parsed) {
  ...
  var kDefaultTitle = "{{order}}";
  skills.forEach(function(skill) {
    var title = (skill.title || kDefaultTitle);
    itemSet.blocks.push({
        type: title.replace("{{order}}", simplifySkillOrder(skill.skills)),
        items: [
          itemFromName("Health Potion"),
          itemFromName("Mana Potion"),
          itemFromName("Stealth Ward"),
          itemFromName("Vision Ward"),
          itemFromName("Warding Totem"),
          itemFromName("Sweeping Lens"),
          itemFromName("Scrying Orb"),
        ]
    });
  });
}
