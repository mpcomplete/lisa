...
function* init() {
  ...
  gTemplate.leagueRootChanged = Async.cb(function*(e, d, sender) {
...
function* load() {
  try {
    var data = yield FS.readFileAsync(kSettingsPath);
...
function* validateLeagueRoot() {
  ...
  try {
    var stats = yield FS.statAsync(gSettings.leagueRoot);
  } catch(e) { }
  ...
  try {
    stats = null;
    stats = yield FS.statAsync(
        Path.join(gSettings.leagueRoot, "RADS", "projects", "lol_air_client"));
  } catch(e) { }

  gTemplate.leagueRootValid = (stats && stats.isDirectory());
  console.log("validated", gSettings.leagueRoot, gTemplate.leagueRootValid);
  return true;
}
